describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for(let i=1;i<prodItems.length;i++){ // skip the first item b/c we already tested that above
      const shadowRoot = await prodItems[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      await button.click();
    }
    const cartCount = await page.$('#cart-count');
    let cartCountValue = await cartCount.getProperty('innerText');
    cartCountValue = await cartCountValue.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    // Check button text of each item
    const prodItems = await page.$$('product-item');
